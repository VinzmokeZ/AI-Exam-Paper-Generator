...
export function GenerateExam() {
  ...
  const handleAIGenerate = async (prompt: string) => {
    ...
    try {
      // Use selectedSubject or extract from prompt if possible
      const subject = selectedSubject;
      // Call API
      await generationService.generateQuestions(subject, prompt, "Mixed", 5);
      toast.success("Exam generated! Redirecting to setup...");
      // For now, allow user to continue flow or redirect to vetting if immediate
      // But GenerateExam seems to be "Creation" phase. 
      // Maybe close prompt and show "Success" state?
      // Or redirect to Vetting center?
      // window.location.href = '/vetting'; // Force reload or use navigate
      // navigate('/vetting');
    } catch (e) {
      console.error(e);
