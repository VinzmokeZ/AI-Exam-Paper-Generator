...
import { vettingService } from '../services/api';
...
export function VettingCenter() {
  ...
  const [selectedCOLevels, setSelectedCOLevels] = useState<any>({});
  ...
  const loadDrafts = async () => {
    try {
      const drafts = await vettingService.getDrafts();
      setQuestions(drafts.length > 0 ? drafts : sampleQuestions);
    } catch (error) {
  ...
  // Handlers need to be defined before returns
  const handleNext = () => {
  ...
  const handleSubmitNext = () => {
    if (currentQuestion && !vettedQuestions[currentQuestion.id]) {
      handleApprove();
    }
    handleNext();
  };
  ...
  if (isLoading) return <div className="text-white text-center mt-20">Loading drafts...</div>;
  if (!currentQuestion) return <div className="text-white text-center mt-20">No questions to review.</div>;
              ...
              <div className="space-y-3 mb-6">
                {currentQuestion.options.map((option: string, index: number) => (
                  <motion.div
                    ...
                    className={`rounded-2xl p-4 border-3 ${index === currentQuestion.correctAnswer
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm ${index === currentQuestion.correctAnswer
                    ...
                    <div className="flex items-center gap-1.5">
                      {[1, 2, 3].map((level) => (
                        <motion.button
                          ...
                          className={`w-10 h-10 rounded-lg font-bold text-sm flex items-center justify-center border-2 transition-all ${currentLevel === level
        ...
        >
          ...
          <div className="flex gap-3 mb-4">
            <motion.button
              ...
              className={`flex-1 rounded-2xl py-4 font-bold text-white flex items-center justify-center gap-2 border-4 ${vettedQuestions[currentQuestion.id] === 'rejected'
            ...
            <motion.button
              ...
              className={`flex-1 rounded-2xl py-4 font-bold text-white flex items-center justify-center gap-2 border-4 ${vettedQuestions[currentQuestion.id] === 'approved'
          ...
          <motion.button
            ...
            className={`w-full rounded-2xl py-5 font-bold text-white flex items-center justify-center gap-3 border-4 border-white/30 relative overflow-hidden ${currentIndex === totalQuestions - 1
      ...
      <div className="mx-6 mb-6 relative z-10">
        <div className="grid grid-cols-2 gap-3">
          <motion.button
            ...
            className={`rounded-2xl py-4 font-bold flex items-center justify-center gap-2 border-4 ${currentIndex === 0
          ...
          <motion.button
            ...
            onClick={handleNext}
            disabled={currentIndex === totalQuestions - 1}
            className={`rounded-2xl py-4 font-bold flex items-center justify-center gap-2 border-4 ${currentIndex === totalQuestions - 1
                ? 'bg-[#F5F1ED] border-[#E5DED6] text-[#8B9E9E] cursor-not-allowed'
                : 'bg-white border-[#E5DED6] text-[#0A1F1F]'
              }`}
          >
