...
import { 
  ...
  Search, Filter, Download, Share2, Bookmark, History
} from 'lucide-react';
...
export function Dashboard() {
  ...
  const currentXP = 8430;
  const nextLevelXP = 10000;
  const xpProgress = (currentXP / nextLevelXP) * 100;

  // Recent Activity
  const recentActivities = [
    { action: 'Generated exam', subject: 'Computer Science', time: '2h ago', color: '#C5B3E6' },
    { action: 'Vetted 15 questions', subject: 'Mathematics', time: '5h ago', color: '#8BE9FD' },
    { action: 'Created rubric', subject: 'Physics', time: '1d ago', color: '#FFB86C' },
  ];

  return (
    <div className="min-h-full relative overflow-hidden">
      {/* Enhanced Floating background elements */}
      <div className="fixed top-10 right-10 w-40 h-40 bg-[#C5B3E6]/10 rounded-full blur-3xl float-slow" />
      <div className="fixed bottom-40 left-10 w-36 h-36 bg-[#6FEDD6]/10 rounded-full blur-3xl float-slow float-delay-1" />
      <div className="fixed top-1/2 right-5 w-32 h-32 bg-[#FFB86C]/10 rounded-full blur-3xl float-slow float-delay-2" />
      <div className="fixed bottom-20 right-20 w-28 h-28 bg-[#FF6AC1]/10 rounded-full blur-3xl float-slow" />
        ...
        >
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1 min-w-0 mr-3">
              ...
              <p className="text-xs text-[#0A1F1F] opacity-70 mb-4 leading-relaxed">Welcome back, Professor</p>
            </div>
            ...
            <div className="flex gap-2 flex-shrink-0">
              <Link to="/notifications">
                <motion.div 
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="w-12 h-12 bg-[#0D2626] rounded-2xl flex items-center justify-center relative"
                >
                  <Bell className="w-5 h-5 text-[#F5F1ED]" />
                  <motion.div
              ...
              <Link to="/settings">
                <motion.div
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="w-12 h-12 bg-[#0D2626] rounded-2xl flex items-center justify-center"
                >
                  <Settings className="w-5 h-5 text-[#F5F1ED]" />
                </motion.div>
              </Link>
            </div>
          </div>
          ...
          <div className="bg-[#0D2626] rounded-3xl p-4 mb-4 relative overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-2 relative z-10">
              <div className="bg-[#0A1F1F] rounded-xl p-2.5 text-center">
                ...
                <p className="text-lg font-bold text-[#8BE9FD] leading-none">1.2k</p>
              </div>
              <div className="bg-[#0A1F1F] rounded-xl p-2.5 text-center">
                <p className="text-[10px] text-[#8B9E9E] leading-tight mb-1">Approval</p>
                <p className="text-lg font-bold text-[#50FA7B] leading-none">92%</p>
              </div>
              <div className="bg-[#0A1F1F] rounded-xl p-2.5 text-center">
                <p className="text-[10px] text-[#8B9E9E] leading-tight mb-1">Exams</p>
                <p className="text-lg font-bold text-[#FFB86C] leading-none">47</p>
              </div>
            </div>
          </div>
          ...
          <Link to="/subjects">
            ...
            >
              ...
              <div className="relative z-10 flex-1">
                ...
                <p className="text-xs text-[#0A1F1F] opacity-70 leading-tight">12 subjects</p>
              </div>
            </motion.div>
          </Link>
          ...
          <Link to="/vetting">
            ...
            >
              ...
              <div className="relative z-10 flex-1">
                ...
                <div className="flex items-center gap-2">
                  <p className="text-xs text-[#0A1F1F] opacity-70 leading-tight">50 pending</p>
                  <motion.div
                    animate={{ scale: [1, 1.2, 1] }}
                    transition={{ duration: 2, repeat: Infinity }}
      ...
      <div className="px-6 pb-6 relative z-10">
        <Link to="/achievements">
          <motion.div
            ...
            whileTap={{ scale: 0.98 }}
            className="bg-gradient-to-br from-[#FF6AC1] via-[#FFB86C] to-[#F1FA8C] rounded-[32px] p-5 border-4 border-white/30 relative overflow-hidden"
          >
            {/* Animated background */}
            <motion.div
              animate={{
      ...
      <motion.button
        ...
        className="fixed bottom-24 right-6 w-16 h-16 bg-gradient-to-br from-[#C5B3E6] to-[#9B86C5] rounded-full flex items-center justify-center shadow-2xl z-40 border-4 border-white/30"
      >
      ...
      <AnimatePresence>
        {showQuickActions && (
          <motion.div
            ...
            className="fixed bottom-44 right-6 bg-[#F5F1ED] rounded-3xl p-4 shadow-2xl z-40 border-4 border-[#E5DED6]"
          >
            <div className="space-y-2 w-48">
              {[
                { icon: Sparkles, label: 'Generate Exam', to: '/generate', color: '#C5B3E6' },
                { icon: Download, label: 'Export Data', to: '#', color: '#8BE9FD' },
                { icon: Calendar, label: 'Schedule', to: '#', color: '#FFB86C' },
                { icon: Share2, label: 'Share', to: '#', color: '#FF6AC1' },
              ].map((action, index) => (
                <motion.button
                  key={action.label}
                  initial={{ opacity: 0, x: 20 }}
