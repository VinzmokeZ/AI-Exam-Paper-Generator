...
export function AIPromptBox({ onGenerate, onClose, engine = 'local', subjectId, subjectName, kbId }: AIPromptBoxProps) {
  ...
  return createPortal(
    <AnimatePresence mode="wait">
      <div className="absolute inset-0 z-[999999] flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="absolute inset-0 bg-[#0A2F2F]/60 backdrop-blur-md cursor-pointer"
          onClick={() => {
            if (!isGenerating && onClose) onClose();
          }}
        />

        {/* Main Content Container to keep particles relative */}
